...
func NewUI(inputChan chan *game.Input, levelChan chan *game.Level) *ui {
	...
	ui.textureAtlas = ui.imgFileToTexture(ui.renderer, "ui2d/assets/tiles.png")
	ui.loadTextureIndex()
...
func (ui *ui) imgFileToTexture(renderer *sdl.Renderer, filename string) *sdl.Texture {
		...
		for event := sdl.PollEvent(); event != nil; event = sdl.PollEvent() {
			...
			case *sdl.WindowEvent:
				if e.Event == sdl.WINDOWEVENT_CLOSE {
					ui.inputChan <- &game.Input{game.CloseWindow, ui.levelChan}
				}
			}
		}
